...
Abstract:
Header for vector, matrix, and quaternion math utility functions useful for 3D
graphics rendering.
*/
...
// Because these are common methods, allow other libraries to overload their
// implementation.
...
/// Returns a vector that is linearly interpolated between the two given
/// vectors.
vector_float3 AAPL_SIMD_OVERLOAD vector_lerp(vector_float3 v0, vector_float3 v1,
                                             float t);

/// Returns a vector that is linearly interpolated between the two given
/// vectors.
vector_float4 AAPL_SIMD_OVERLOAD vector_lerp(vector_float4 v0, vector_float4 v1,
                                             float t);

/// Converts a unit-norm quaternion into its corresponding rotation matrix.
matrix_float3x3 AAPL_SIMD_OVERLOAD
matrix3x3_from_quaternion(quaternion_float q);

/// Constructs a matrix_float3x3 from three rows of three columns with float
/// values. Indices are m<column><row>.
matrix_float3x3 AAPL_SIMD_OVERLOAD matrix_make_rows(float m00, float m10,
                                                    float m20, float m01,
                                                    float m11, float m21,
                                                    float m02, float m12,
                                                    float m22);

/// Constructs a matrix_float4x4 from four rows of four columns with float
/// values. Indices are m<column><row>.
matrix_float4x4 AAPL_SIMD_OVERLOAD matrix_make_rows(
    float m00, float m10, float m20, float m30, float m01, float m11, float m21,
...
matrix_float3x3 AAPL_SIMD_OVERLOAD matrix3x3_rotation(float radians,
                                                      vector_float3 axis);

/// Constructs a rotation matrix from the given angle and axis.
matrix_float3x3 AAPL_SIMD_OVERLOAD matrix3x3_rotation(float radians, float x,
                                                      float y, float z);
...
/// Constructs a scaling matrix, using the given vector as an array of scaling
/// factors.
...
matrix4x4_from_quaternion(quaternion_float q);

/// Constructs a rotation matrix from the provided angle and axis
matrix_float4x4 AAPL_SIMD_OVERLOAD matrix4x4_rotation(float radians,
                                                      vector_float3 axis);

/// Constructs a rotation matrix from the given angle and axis.
matrix_float4x4 AAPL_SIMD_OVERLOAD matrix4x4_rotation(float radians, float x,
                                                      float y, float z);
...
matrix_float4x4 AAPL_SIMD_OVERLOAD matrix4x4_scale(float sx, float sy,
                                                   float sz);
...
matrix_float4x4 AAPL_SIMD_OVERLOAD matrix4x4_translation(float tx, float ty,
                                                         float tz);

/// Constructs a translation matrix that translates by the vector (t.x, t.y,
/// t.z).
...
matrix_float4x4 AAPL_SIMD_OVERLOAD matrix4x4_scale_translation(vector_float3 s,
                                                               vector_float3 t);

/// Starting with left-hand world coordinates, constructs a view matrix that is
/// positioned at (eyeX, eyeY, eyeZ) and looks toward (centerX, centerY,
/// centerZ), with the vector (upX, upY, upZ) pointing up for a left-hand
/// coordinate system.
matrix_float4x4 AAPL_SIMD_OVERLOAD matrix_look_at_left_hand(
...
matrix_float4x4 AAPL_SIMD_OVERLOAD matrix_look_at_left_hand(
    vector_float3 eye, vector_float3 target, vector_float3 up);

/// Starting with right-hand world coordinates, constructs a view matrix that is
/// positioned at (eyeX, eyeY, eyeZ) and looks toward (centerX, centerY,
/// centerZ), with the vector (upX, upY, upZ) pointing up for a right-hand
/// coordinate system.
matrix_float4x4 AAPL_SIMD_OVERLOAD matrix_look_at_right_hand(
    float eyeX, float eyeY, float eyeZ, float centerX, float centerY,
    float centerZ, float upX, float upY, float upZ);
...
/// That maps (left, top) to (-1, 1), (right, bottom) to (1, -1), and (nearZ,
/// farZ) to (0, 1). The first four arguments are signed eye coordinates. nearZ
/// and farZ are absolute distances from the eye to the near and far clip
/// planes.
matrix_float4x4 AAPL_SIMD_OVERLOAD matrix_ortho_left_hand(
...
/// and farZ are absolute distances from the eye to the near and far clip
/// planes.
matrix_float4x4 AAPL_SIMD_OVERLOAD matrix_ortho_right_hand(
    float left, float right, float bottom, float top, float nearZ, float farZ);
...
matrix_float4x4 AAPL_SIMD_OVERLOAD matrix_perspective_left_hand(
...
/// fovyRadians, the given aspect ratio, and the given absolute near and far
/// z distances from the eye.
matrix_float4x4 AAPL_SIMD_OVERLOAD matrix_perspective_right_hand(
    float fovyRadians, float aspect, float nearZ, float farZ);

/// Construct a general frustum projection matrix, from right-hand eye
/// coordinates to left-hand clip coordinates.
/// The bounds left, right, bottom, and top, define the visible frustum at the
/// near clip plane. The first four arguments are signed eye coordinates. nearZ
/// and farZ are absolute distances from the eye to the near and far clip
/// planes.
matrix_float4x4 AAPL_SIMD_OVERLOAD matrix_perspective_frustum_right_hand(
    float left, float right, float bottom, float top, float nearZ, float farZ);
...
quaternion_float AAPL_SIMD_OVERLOAD quaternion(float x, float y, float z,
                                               float w);
...
/// Constructs a unit-norm quaternion that represents rotation by the given
/// angle about the axis (x, y, z).
quaternion_float AAPL_SIMD_OVERLOAD quaternion(float radians, float x, float y,
                                               float z);

/// Constructs a unit-norm quaternion that represents rotation by the given
/// angle about the specified axis.
quaternion_float AAPL_SIMD_OVERLOAD quaternion(float radians,
                                               vector_float3 axis);
...
quaternion_float AAPL_SIMD_OVERLOAD
quaternion_from_axis_angle(vector_float3 axis, float radians);

/// Returns a quaternion from the given 3x3 rotation matrix.
quaternion_float AAPL_SIMD_OVERLOAD
quaternion_from_matrix3x3(matrix_float3x3 m);
...
quaternion_float AAPL_SIMD_OVERLOAD quaternion_multiply(quaternion_float q0,
                                                        quaternion_float q1);

/// Returns the quaternion that results from spherically interpolating between
/// the two given quaternions.
quaternion_float AAPL_SIMD_OVERLOAD quaternion_slerp(quaternion_float q0,
                                                     quaternion_float q1,
                                                     float t);

/// Returns the vector that results from rotating the given vector by the given
/// unit-norm quaternion.
vector_float3 AAPL_SIMD_OVERLOAD quaternion_rotate_vector(quaternion_float q,
                                                          vector_float3 v);

/// Returns the quaternion for the given forward and up vectors for right-hand
/// coordinate systems.
quaternion_float AAPL_SIMD_OVERLOAD
quaternion_from_direction_vectors_right_hand(vector_float3 forward,
                                             vector_float3 up);

/// Returns the quaternion for the given forward and up vectors for left-hand
/// coordinate systems.
quaternion_float AAPL_SIMD_OVERLOAD quaternion_from_direction_vectors_left_hand(
    vector_float3 forward, vector_float3 up);

/// Returns a vector in the +Z direction for the given quaternion.
vector_float3 AAPL_SIMD_OVERLOAD
forward_direction_vector_from_quaternion(quaternion_float q);

/// Returns a vector in the +Y direction for the given quaternion (for a
/// left-handed coordinate system,
///   negate for a right-hand coordinate system).
vector_float3 AAPL_SIMD_OVERLOAD
up_direction_vector_from_quaternion(quaternion_float q);

/// Returns a vector in the +X direction for the given quaternion (for a
/// left-hand coordinate system,
///   negate for a right-hand coordinate system).
vector_float3 AAPL_SIMD_OVERLOAD
right_direction_vector_from_quaternion(quaternion_float q);
